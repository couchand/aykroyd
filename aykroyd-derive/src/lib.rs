...
pub fn derive_pg_enum(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    quote! {
        ...
        impl<'a> ::aykroyd::types::FromSql<'a> for #rust_name {
            fn from_sql(_: &::aykroyd::types::Type, buf: &'a [u8]) -> ::std::result::Result<Self, ::std::boxed::Box<(dyn ::std::error::Error + Send + Sync + 'static)>> {
                match buf {
                    #(#from_sql_cases)*
            ...
            fn accepts(ty: &::aykroyd::types::Type) -> bool {
                ty.name() == #pg_name
            }
        }

        #[automatically_derived]
        impl ::aykroyd::types::ToSql for #rust_name {
            fn to_sql(&self, _: &::aykroyd::types::Type, buf: &mut ::aykroyd::types::BytesMut) -> ::std::result::Result<::aykroyd::types::IsNull, ::std::boxed::Box<(dyn ::std::error::Error + Send + Sync + 'static)>> {
                match self {
                    #(#to_sql_cases)*
                }
                Ok(::aykroyd::types::IsNull::No)
            }
            ...
            ::aykroyd::types::to_sql_checked!();
        }

        #[automatically_derived]
        impl ::aykroyd::PgEnum for #rust_name {}
    }.into()
}
...
pub fn derive_from_row(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    proc_macro::TokenStream::from(quote! {
        #[automatically_derived]
        impl ::aykroyd::FromRow for #name {
            fn from_row(row: ::aykroyd::types::Row) -> ::std::result::Result<Self, ::aykroyd::types::Error> {
                #body
            }
        }
    })
...
fn derive_statement_impl(ast: &syn::DeriveInput) -> proc_macro2::TokenStream {
    ...
    quote! {
        #[automatically_derived]
        impl #generics ::aykroyd::Statement for #name #generics {
            const TEXT: &'static str = #query;

            type Row = #output;

            fn to_row(&self) -> Vec<&(dyn ::aykroyd::types::ToSql + Sync)> {
                let mut res = vec![];

                #(
                    res.push(&self.#fields as &(dyn ::aykroyd::types::ToSql + Sync));
                )*
